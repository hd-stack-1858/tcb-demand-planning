...
#   -------   -----------------------   -----------------   ----------------------------
#   dev       tcb-growthspurt-dev       growthspurt-dev     supabase-url-dev / supabase-key-dev
#   staging   tcb-growthspurt-staging   growthspurt-stg     supabase-url-staging / supabase-key-staging
#   prod      tcb-growthspurt           growthspurt         supabase-url / supabase-key
#
...
case "${ENV}" in
  dev)
    SERVICE=tcb-growthspurt-dev
    IMAGE_NAME=growthspurt-dev
    SECRET_URL=supabase-url-dev
    SECRET_KEY=supabase-key-dev
    ;;
  staging)
    SERVICE=tcb-growthspurt-staging
    IMAGE_NAME=growthspurt-stg
    SECRET_URL=supabase-url-staging
    SECRET_KEY=supabase-key-staging
    ;;
  prod)
    SERVICE=tcb-growthspurt
    IMAGE_NAME=growthspurt
...
echo "==> Deploying Growth Spurt to env=${ENV}, service=${SERVICE}"
...
gcloud run deploy "${SERVICE}" \
  ...
  --image="${IMAGE}" \
  --port=8080 \
  --set-env-vars=STREAMLIT_APP=growthspurt_app.py \
