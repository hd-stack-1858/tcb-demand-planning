"""Contract tests for the Growth Spurt Cloud Run deploy script (issues #116, #131, epic #113).

Guard the Cloud Run deploy contract for ui/growthspurt_app.py: the deploy
script must exist, be executable, support dev/staging/prod environments with
env-specific service names and secrets, inject STREAMLIT_APP=growthspurt_app.py,
...
DEPLOY_SCRIPT = REPO_ROOT / "scripts" / "gcp" / "deploy_growthspurt.sh"
...
def test_deploy_script_exists():
    assert DEPLOY_SCRIPT.exists(), "scripts/gcp/deploy_growthspurt.sh is missing"


def test_deploy_script_is_executable():
    mode = DEPLOY_SCRIPT.stat().st_mode
    assert mode & stat.S_IXUSR, "deploy_growthspurt.sh must be executable (chmod +x)"


def test_sets_streamlit_app_env_var():
    # STREAMLIT_APP must point to the Growth Spurt app, not TinySteps
    assert "STREAMLIT_APP=growthspurt_app.py" in _script_text()


def test_no_allow_unauthenticated_flag_present():
    # Growth Spurt is Sales MIS — real order data. Must never be publicly reachable.
...
def test_prod_env_uses_unqualified_service_name():
    # Backward compat: prod service is tcb-growthspurt (no suffix)
    assert "SERVICE=tcb-growthspurt\n" in _script_text()


def test_staging_env_uses_staging_service_name():
    assert "SERVICE=tcb-growthspurt-staging" in _script_text()


def test_dev_env_uses_dev_service_name():
    assert "SERVICE=tcb-growthspurt-dev" in _script_text()
