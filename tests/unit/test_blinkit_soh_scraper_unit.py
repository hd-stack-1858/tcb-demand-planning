"""Unit tests for automation/blinkit_soh_scraper.py's session-store wiring.

Mirrors test_blinkit_scraper_unit.py's depth — full read-session ->
scrape -> write-back-session flow, fully mocked (no browser, no network,
no real files, no credentials).
"""
...
import automation.blinkit_soh_scraper as soh_scraper
...
def test_raises_before_launching_browser_when_no_session_saved(monkeypatch):
    ...
    monkeypatch.setattr(soh_scraper, "sync_playwright", mock_playwright)
...
def test_passes_loaded_session_through_and_refreshes_on_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(soh_scraper, "DOWNLOAD_DIR", tmp_path)
    monkeypatch.setattr(soh_scraper, "_is_login_page", lambda page: False)
    monkeypatch.setattr(soh_scraper.time, "sleep", lambda *a, **k: None)

    p = MagicMock()
    cm = MagicMock()
    cm.__enter__.return_value = p
    cm.__exit__.return_value = False
    monkeypatch.setattr(soh_scraper, "sync_playwright", MagicMock(return_value=cm))
    ...
    dest = soh_scraper.scrape()

    # session_state (from the store) was handed to new_context via _new_browser_context
    ...
    # refreshed storage_state was written back to the store
