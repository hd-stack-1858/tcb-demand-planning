...
def _load_wh_sellable_stock(sku_id: str) -> dict:
    ...
    rows = (db.table("blinkit_inventory_snapshots")
              ...
              .eq("sku_id", sku_id)
              .execute().data)
    return {r["location_id"]: (r.get("total_sellable") or 0) for r in rows}
...
def tab_blinkit_deepdive(fdf: pd.DataFrame, net_mode: bool) -> None:
    ...
    def _serving_wh_and_stock(grp: pd.DataFrame) -> tuple[str, int]:
        ...
        total_stock = sum(wh_stock.get(pid, 0) for pid in parent_ids)
    ...
    def _city_count_df(rows: list[tuple]) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            "Total Sellable Inventory": [stock for _, _, _, stock in rows],
        })
