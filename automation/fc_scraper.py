...
Flow per run:
  ...
  4. Email all downloaded PDFs (Invoice + Packing Slip for each order) to Himanshu + Meet
...
def run(dry_run: bool = False, headed: bool = False, no_email: bool = False) -> dict:
    ...
    if not recipients:
        logger.warning("No email recipients. Set EMAIL_HIMANSHU / EMAIL_MEET in .env")
    else:
